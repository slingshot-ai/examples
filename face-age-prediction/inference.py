...
class FaceAgeDeployment(InferenceModel):
    ...
    def get_face_age_model(self) -> nn.Module:
        ...
        self.face_age_model.fc = nn.Linear(self.face_age_model.fc.in_features, self.config.num_age_classes)
        self.face_age_model = self.face_age_model.to(DEVICE)

        face_age_checkpoint_path = self.config.checkpoint_path
        checkpoint: Dict[str, torch.Tensor] = torch.load(face_age_checkpoint_path, map_location=DEVICE)
    ...
    def _detect_face_and_crop_square(self, img, img_h, img_w) -> list[int]:
        ...
        x1, y1, x2, y2 = faces[0, 0, 0, 3:7] * np.array([img_h, img_w, img_h, img_w])
        x1, y1, x2, y2 = round(x1), round(y1), round(x2), round(y2)
        w, h = abs(x2 - x1), abs(y2 - y1)
        return [x1, y1, x2, y2, w, h]

    @torch.inference_mode()
    def _predict_age(self, img, img_size=224, margin=0.4) -> float:
        ...
        cv2.rectangle(img, (left_extended, top_extended), (right_extended, bottom_extended), (255, 0, 0), 2)

        # crop the face and resize it to the model's required input size
        try:
            inputs = cv2.resize(
                img[top_extended : bottom_extended + 1, left_extended : right_extended + 1,], (img_size, img_size)
            )
        ...
        ages = torch.arange(0, self.config.num_age_classes).to('cuda').float()
    ...
    async def load(self) -> None:
        self.config = DeployConfig.parse_obj(json.loads(os.environ.get("CONFIG", "{}")))

        config_path = os.path.join(self.face_detection_model_path, self.config.face_detection_config_name)
        detection_model_path = os.path.join(self.face_detection_model_path, self.config.face_detection_model_name)
        self.face_detection_net = cv2.dnn.readNetFromCaffe(config_path, detection_model_path)
    ...
    async def predict(self, examples: list[bytes]) -> dict[str | float]:
        try:
            ...
            image_examples = examples["image"]
            image_bytes = b64decode(image_examples)
        except Exception as e:
            # when using the 'Test' from deployment, the examples are not in json format
