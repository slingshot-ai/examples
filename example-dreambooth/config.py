...
class DreamboothConfigTrain(BaseModel):
    ...
    # file under pretrain_model_name_or_path path). So, the pretrain_model_name_or_path is now set to this such that the
    # base model can be picked.
    ...
    # Whether to resume training from a checkpoint. If False, the model will be trained from scratch. If True, the model
    # will be trained from the last saved checkpoint if the base model is provided.
    ...
    # The resolution of images that are expected by the model. Currently, testing has been done on resolution 512 model
    # itself and the SD 1.5 model used is fine-tuned on 512x512 resolution images.
    resolution: int = 512
    ...
    @field_validator('base_model_path')
    def check_base_model_path(cls, base_model_path: str):
        if base_model_path == "":
            raise ValueError(f"Please set MODEL_PATH correctly.")
