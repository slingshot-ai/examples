...
# Reference: Code adapted from https://github.com/royorel/Lifespan_Age_Transformation_Synthesis
import torch
...
def define_D(input_nc, ndf, n_layers=6, numClasses=2, gpu_ids=[], init_type='gaussian'):
    netD = StyleGANDiscriminator(
        input_nc, ndf=ndf, n_layers=n_layers, numClasses=numClasses
    )
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, latent):
        ...
        out = (
            self.conv(input, weight, groups=b).view(b, self.out_channels, h, w)
            + self.bias
        )
...
class MLP(nn.Module):
    def __init__(
        ...
        # set the first layer
        self.model += [linear(input_dim, fc_dim), actvn]
        if normalize_mlp:
            self.model += [PixelNorm()]

        # set the inner layers
        for i in range(n_fc - 2):
            self.model += [linear(fc_dim, fc_dim), actvn]
            if normalize_mlp:
                self.model += [PixelNorm()]
...
class AgeEncoder(nn.Module):
    def __init__(
        ...
        encoder += [
            conv2d(ngf * mult * 2, style_dim, kernel_size=1, stride=1, padding=0)
        ]
...
class StyledDecoder(nn.Module):
    def __init__(
        ...
        self.StyledConvBlock_0 = StyledConvBlock(
            ngf * mult,
            ngf * mult,
            latent_dim=latent_dim,
        ...
        self.StyledConvBlock_1 = StyledConvBlock(
            ngf * mult,
            ngf * mult,
            latent_dim=latent_dim,
            padding=padding_type,
        ...
        self.StyledConvBlock_2 = StyledConvBlock(
            ngf * mult,
            ngf * mult,
            latent_dim=latent_dim,
            padding=padding_type,
        ...
        self.StyledConvBlock_3 = StyledConvBlock(
            ngf * mult,
            ngf * mult,
            latent_dim=latent_dim,
            padding=padding_type,
        ...
        self.StyledConvBlock_up0 = StyledConvBlock(
            ngf * mult,
            ngf * mult // 2,
            latent_dim=latent_dim,
            padding=padding_type,
        ...
        self.StyledConvBlock_up1 = StyledConvBlock(
            ngf * mult // 2,
            last_upconv_out_layers,
            latent_dim=latent_dim,
            padding=padding_type,
            upsample=True,
            actvn=actvn,
        ...
        self.conv_img = nn.Sequential(
            EqualConv2d(last_upconv_out_layers, output_nc, 1), nn.Tanh()
        )
        self.mlp = MLP(
            style_dim,
            latent_dim,
    ...
    def forward(
        ...
        if traverse:
            ...
            for i in range(orig_class_num - 1):
                latent[interps * i : interps * (i + 1), :] = (
                    alphas * temp_latent[i, :] + (1 - alphas) * temp_latent[i + 1, :]
                )
...
class Generator(nn.Module):
    def __init__(
        ...
        self.id_encoder = IdentityEncoder(
            ...
            conv_weight_norm=conv_weight_norm,
            actvn='relu',
        )  # replacing relu with leaky relu here causes nans and the entire training to collapse immediately
    ...
    # parallel forward
    def forward(
        self, input, target_age_code, cyc_age_code, source_age_code, disc_pass=False
    ):
    ...
    def infer(
        self, input, target_age_features, traverse=False, deploy=False, interp_step=0.5
    ):
        id_features = self.id_encoder(input)
        out = self.decode(
            id_features,
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        self.conv_block = self.build_conv_block(
            dim, padding_type, norm_layer, activation, conv_weight_norm, use_pixel_norm
        )
    ...
    def build_conv_block(
        ...
        conv_block += [
            conv2d(dim, dim, kernel_size=3, padding=p),
            norm_layer(dim),
            activation,
        ]
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [conv2d(dim, dim, kernel_size=3, padding=p), norm_layer(dim)]
...
class StyleGANDiscriminator(nn.Module):
    def __init__(
        self, input_nc, ndf=64, n_layers=6, numClasses=2, padding_type='reflect'
    ):
        ...
        for n in range(n_layers):
            ...
            nf = min(nf * 2, 512)
            sequence += [
                StyledConvBlock(nf_prev, nf, downsample=True, actvn=activation)
            ]
